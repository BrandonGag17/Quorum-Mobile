...
export default function ProponerJuntada() {
  ...
  const { crearPropuesta, cargando, error, limpiarError } = useCreateProposal();
  ...
  useEffect(() => {
    ...
    const nuevosLugares = recomendados.map((lugar) =>
      [lugar.nombre, lugar.direccion].filter(Boolean).join(" — "),
    );

    setOpcionesLugares((actuales) => [
      ...actuales,
      ...nuevosLugares.filter((lugar) => lugar && !actuales.includes(lugar)),
    ]);
  ...
  const [fechaTextoTemporal, setFechaTextoTemporal] = useState("");
  ...
  const [fechaCierreDate, setFechaCierreDate] = useState(new Date());
  ...
  const formatearFecha = (fechaObjeto) => {
    ...
    const hora =
      ...
      `${fechaObjeto.getMinutes().toString().padStart(2, "0")}`;
  ...
  const onChangeNativo = (event, selectedDate) => {
    ...
    if (Platform.OS === "android") {
      ...
      setFechaTextoTemporal(formatearFecha(currentDate));
  ...
  const añadirFecha = () => {
    ...
    if (Platform.OS === "web" && fechaWeb) {
      const [fecha, hora] = fechaWeb.split("T");
      ...
      nuevaFecha = fechaTextoTemporal;
    }

    if (nuevaFecha && !opcionesFechas.includes(nuevaFecha)) {
      setOpcionesFechas([...opcionesFechas, nuevaFecha]);
  ...
  const eliminarFecha = (fecha) => {
    setOpcionesFechas(opcionesFechas.filter((item) => item !== fecha));
  };

  const añadirLugar = () => {
    const lugar = lugarTemporal.trim();

    if (lugar && !opcionesLugares.includes(lugar)) {
      setOpcionesLugares([...opcionesLugares, lugar]);
  ...
  const eliminarLugar = (lugar) => {
    setOpcionesLugares(opcionesLugares.filter((item) => item !== lugar));
  };

  const validarPaso1 = () => {
    limpiarError();

    const nombre = nombreJuntada.trim();
  ...
  const validarPaso2 = () => {
    ...
    if (opcionesFechas.length === 0 && opcionesLugares.length === 0) {
      return;
    }
    ...
    return (
      <View style={styles.fondo}>
        ...
        <View style={styles.formulario}>
          <Text style={styles.titulo}>Crear propuesta de juntada</Text>

          <Text style={styles.descripcionPaso}>
            Primero contanos qué juntada querés organizar.
          </Text>
  ...
  if (paso === 2) {
    return (
      <View style={styles.fondo}>
        ...
        >
          <Text style={styles.titulo}>Elegí las opciones</Text>

          <Text style={styles.descripcionPaso}>
            Podés proponer distintas fechas, horarios y lugares para que el
            grupo vote.
          </Text>
          ...
          {Platform.OS === "web" ? (
            ...
            >
              <Text style={styles.selectorTexto}>
                {fechaTextoTemporal || "Seleccionar fecha y hora"}
              </Text>
            </Pressable>
          )}
          ...
          {opcionesFechas.length > 0 && (
            <View style={styles.lista}>
              {opcionesFechas.map((fecha, index) => (
                <View
                  key={fecha}
                  style={[
                    styles.item,
                ...
                >
                  <Text style={styles.itemTexto}>{fecha}</Text>

                  <Pressable
                    onPress={() => eliminarFecha(fecha)}
          ...
          {opcionesLugares.length > 0 && (
            <View style={styles.lista}>
              {opcionesLugares.map((lugar, index) => (
                <View
                  key={lugar}
                  style={[
                    styles.item,
                ...
                >
                  <Text style={styles.itemTexto}>{lugar}</Text>

                  <Pressable
                    onPress={() => eliminarLugar(lugar)}
                    style={styles.botonEliminar}
                  >
                    <MaterialCommunityIcons
                      name="trash-can-outline"
  ...
  return (
    <View style={styles.fondo}>
      ...
      <View style={styles.formulario}>
        <Text style={styles.titulo}>Cierre de la votación</Text>

        <Text style={styles.descripcionPaso}>
          Elegí hasta cuándo los integrantes del grupo podrán votar.
        </Text>

        {Platform.OS === "web" ? (
          <input
            type="datetime-local"
            value={fechaCierre ? fechaCierre.slice(0, 16) : ""}
          ...
          <View style={styles.datePickerContainer}>
            <DateTimePicker
              ...
              onChange={(event, selectedDate) => {
                ...
                if (selectedDate) {
                  setFechaCierreDate(selectedDate);
      ...
      <View style={styles.botonContainer}>
        ...
        <ButtonApp
          nombre={cargando ? "Creando..." : "Crear propuesta"}
